        ...
        if ($array_with_tariffs) {
            ...
            foreach ($current_cat as $channel_array) {
                foreach ($channel_array as $key => $value) //перебор массива + запрос
                {
                    ...
                    if (array_key_exists($value, $array_with_tariffs)) { //получаем категорию для выделения синим
                        $channel_list[$j]['category'] = $array_with_tariffs[$value];
                    } else {
                        $channel_list[$j]['category'] = 0; //если вдруг в json-массиве нет такого канала - присваиваем ему категорию "0"
                    }
            ...
            $channel_list = collect($channel_list)->sortBy('number'); //коллекционируем массив
