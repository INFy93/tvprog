...
class LogoController extends Controller
{
    ...
    { //отображаем табличку с логотипами
        $logo = Itv::leftJoin('logos', function ($join) {
    ...
    public function oneLogoToChange($id)
    { //страничка изменения логотипа
        $logo_single = Itv::leftJoin('logos', function ($join) {
            $join->on('itv.id', '=', 'logos.ch_id');
        })
            ->where('itv.id', '=', $id)
    ...
    public function oneLogoToEdit($id)
    { //страничка добавления логотипа
        $logo_single = Itv::leftJoin('logos', function ($join) {
            $join->on('itv.id', '=', 'logos.ch_id');
        })
            ->where('itv.id', '=', $id)
    ...
    public function updateLogo($id, LogoRequest $request)
