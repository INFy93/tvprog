...
class ProgrammList
{ //формируем телепрограмму
    public static function getBeforeProgramm($id)
    { //получаем передачи, время которых еще не пришло
    ...
    { //получаем последующие передачи
        $time = date("Y-m-d H:i:s");
        if ($limit_key == 1) { //выводим 4 передачи для показа на главной странице
            $epg = Epg::select('id', 'ch_id', 'time', 'time_to', 'name', 'descr')
                ->where([
                    ['ch_id', '=', $id],
                    ['time', '>', $time]
                ...
                ->orderBy('time', 'asc')
                ->get();
        } else if ($limit_key == 0) { //выводим все передачи, что есть в базе - для страницы с полной программой
            $epg = Epg::select('id', 'ch_id', 'time', 'time_to', 'name', 'descr')
                ->where([
                    ['ch_id', '=', $id],
                    ['time', '>', $time]
