                ...
                <li class="nav-item">
                    ...
                    <a href="#"
                            ...
                            title="Применяет темную или светлую тему для общей программы и программы конкретного канала. По умолчанию - светлая тема."><i
                                class='fas fa-question-circle' style='font-size:24px; vertical-align: middle; paddnig-right: 5px;'></i></a> </label>
                </li>
                <hr>
                <li class="nav-item">
            ...
            <div
                ...
                <h5 class="channel_title">
                    <img class="logo" src="/storage/{{ $channel->logo_path }}"> <a
                        ...
                        @if (Auth::check()) <a
                                href="{{ route('logo_data', $channel->channel_id) }} " class="badge badge-info">Изменить
                                лого</a>@endif
                    </a>
                </h5>
                    ...
                    @foreach (ProgrammList::getBeforeProgramm($channel->channel_id) as $progB)
                        <div class="before">
                            ...
                            @if (strlen($progB->descr) != 0)
                                <a href="#" id="{{ $progB->id }}" class="load_description" data-toggle="modal"
                                data-target="#basicModal" onClick="description(this);">{{ $progB->name }}</a> @else
                                {{ $progB->name }}
                            @endif
                        </div>
                    @endforeach
                    <div class="current">
                        ...
                        @if (strlen($programCurrent['epg']['descr']) != 0)
                            <a href="#" id="{{ $programCurrent['epg']->id }}" class="load_description" data-toggle="modal"
                                ...
                                onClick="descriptionCurrent(this);">{{ $programCurrent['epg']['name'] }}</a>
                            <div class="progress" style="height: 4px;">
                                <div class="progress-bar" role="progressbar"
                                    style="width: {{ $programCurrent['progress'] }}%"
                    ...
                    @foreach (ProgrammList::getAfterProgramm($channel->channel_id, 1) as $progA)
                        <div class="after">
                            ...
                            @if (strlen($progA->descr) != 0)
                                <a href="#" id="{{ $progA->id }}" class="load_description" data-toggle="modal"
                                data-target="#basicModal" onClick="description(this);">{{ $progA->name }}</a> @else
                                {{ $progA->name }}
                            @endif
                        </div>
                    @endforeach
